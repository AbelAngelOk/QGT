...
export function Projects () {
    ...
    const urlImagenes = [logoSeguridad, logoEducation, logoImpositivo, logoSalud, logoNull]
                        ...
                        <ul className='proyectos--containerBox__ul'>
                            { 
                                projects.map( (p) => {
                                    ...
                                    return (
                                        <li className={`proyectos--box ${cajaIluminada === p.id ? "sombreado" : "" }`} onClick={ () => manejarClick(<ContenidoDetalleProyecto titulo={p.titulo} descripccion={p.description} imagen={urlImagenes[p.id]} />, p.id) }
                                    > { contenido_caja_de_seccion_proyectos(urlImagenes[p.id], p.titulo) } </li> 
                                    )
                                })
                            }
                        </ul>

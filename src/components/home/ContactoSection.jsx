...
function Contacto () {
  return (
    <section id='contacto' className='w-full h-auto py-5 bg-slate-50 dark:bg-slate-900 flex flex-wrap p-2'>
      <iframe className='w-[40%] min-w-[500px]  h-4/6' src={contactValues.UrlLocation} style='border:0;' allowfullscreen='' loading='lazy' referrerpolicy='no-referrer-when-downgrade' />
      <div id='contacto-info' className='w-[45%] min-w-[500px] h-5/6 justify-between items-center'>
        <div id='contacto-up' className='w-full h-1/2 text-center'>
          <h4 id='contacto-up-title' className='text-2xl font-bold uppercase px-4'> {contactValues.tituloInfo} </h4>
        ...
        <div id='contacto-down' className='w-full h-1/2 text-center'>
          ...
          <form className='w-full'>
            <input type='text' className='w-2/5 border' placeholder='nombre' required />
            <input type='email' className='w-2/5 border' placeholder='email' required />
            <input type='text' className='w-full border' placeholder='mensaje' required />
